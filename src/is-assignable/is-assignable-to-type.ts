...
export function isAssignableToType(
	...
	options = {
		...(checkerOrOptions === undefined ? {} : isProgram(checkerOrOptions) ? checkerOrOptions.getCompilerOptions() : isTypeChecker(checkerOrOptions) ? {} : checkerOrOptions),
		...(options || {}),
	}
	...
	if (!isSimpleType(typeA) && !isSimpleType(typeB) && checker !== undefined && (checker as TypeCheckerWithInternals).isTypeAssignableTo !== undefined) {
