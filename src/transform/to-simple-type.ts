...
function namedMember(symbol: ts.Symbol, memberOfType: Type, options: ToSimpleTypeInternalOptions): SimpleTypeMemberNamed {
	...
	const modifiers = declaration !== undefined ? getModifiersFromDeclaration(declaration, ts) : []
	if (modifiers.length) {
...
function toSimpleTypeInternal(type: Type, options: ToSimpleTypeInternalOptions): SimpleType {
	...
	const name = symbol !== undefined ? getRealSymbolName(symbol, ts) : undefined
	...
	if (isLiteral(type, ts)) {
		...
		if (literalSimpleType !== undefined) {
			// Enum members
			if (symbol !== undefined && symbol.flags & ts.SymbolFlags.EnumMember) {
	...
	// Array
		...
		const aliasedName = type.aliasSymbol !== undefined ? getRealSymbolName(type.aliasSymbol, ts) : undefined
		simpleType = {
			kind: "ARRAY",
			type: toSimpleTypeCached(getTypeArguments(type, checker, ts)[0], options),
	...
	// Class
	else if (type.isClass() && symbol !== undefined) {
		const classDecl = getDeclaration(symbol, ts)

		if (classDecl !== undefined && ts.isClassDeclaration(classDecl)) {
			const ctor = (() => {
				const ctorSymbol = symbol !== undefined && symbol.members !== undefined ? symbol.members.get("__constructor" as never) : undefined
				if (ctorSymbol !== undefined && symbol !== undefined) {
					...
					if (ctorDecl !== undefined && ts.isConstructorDeclaration(ctorDecl)) {
	...
	// Interface
		...
		const typeParameters =
			(type.isClassOrInterface() && type.typeParameters !== undefined ? type.typeParameters.map(t => toSimpleTypeCached(t, options) as SimpleTypeGenericParameter) : undefined) ||
			(symbol !== undefined ? getTypeParameters(getDeclaration(symbol, ts), options) : undefined)
		...
		/*if (call!== undefined && members.length === 0 && ctor === undefined && indexType === undefined) {
	...
	// Function
	else if (symbol !== undefined && (isFunction(type, ts) || isMethod(type, ts))) {
	...
	// Type Parameter
	else if (type.isTypeParameter() && symbol !== undefined) {
		// This type
		if (isThisType(type, ts) && symbol.valueDeclaration !== undefined) {
		...
		const constraintSimpleType = constraint !== undefined ? toSimpleTypeCached(constraint, options) : undefined
		const defaultSimpleType = defaultType !== undefined ? toSimpleTypeCached(defaultType, options) : undefined
...
function primitiveLiteralToSimpleType(type: Type, checker: TypeChecker, ts: TsModule): SimpleTypeLiteral | undefined {
...
function getSimpleFunctionFromSignatureDeclaration(
	...
	const parameters = signatureDeclaration.parameters.map(parameterDecl => {
		...
		return {
			...
			optional: parameterDecl.questionToken !== undefined,
			type: toSimpleTypeCached(argType, options),
			rest: parameterDecl.dotDotDotToken !== undefined,
			initializer: parameterDecl.initializer !== undefined,
		} as SimpleTypeFunctionParameter
	})

	const name = symbol !== undefined ? symbol.getName() : undefined
	...
	const returnType = signature === undefined ? undefined : toSimpleTypeCached(checker.getReturnTypeOfSignature(signature), options)
...
function getRealSymbolName(symbol: ESSymbol, ts: TsModule): string | undefined {
	const name = symbol.getName()
	if (name !== undefined && [ts.InternalSymbolName.Type, ts.InternalSymbolName.Object, ts.InternalSymbolName.Function].includes(name as never)) {
		return undefined
	}
...
function getTypeParameters(obj: ESSymbol | Declaration | undefined, options: ToSimpleTypeInternalOptions): SimpleTypeGenericParameter[] | undefined {
	...
	if (
		...
		return obj.typeParameters === undefined
			? undefined
