...
export function isTypeChecker(obj: unknown): obj is TypeChecker {
	return obj !== undefined && typeof obj === "object" && "getSymbolAtLocation" in obj!
}

export function isProgram(obj: unknown): obj is Program {
	return obj !== undefined && typeof obj === "object" && "getTypeChecker" in obj! && "getCompilerOptions" in obj!
}

export function isNode(obj: unknown): obj is Node {
	return obj !== undefined && typeof obj === "object" && "kind" in obj! && "flags" in obj! && "pos" in obj! && "end" in obj!
}
...
export function isFunction(type: Type, ts: typeof tsModule): type is ObjectType {
	...
	return (symbol.flags & ts.SymbolFlags.Function) !== 0 || symbol.escapedName === "Function" || (symbol.members !== undefined && symbol.members.has("__call" as never))
}
...
export function getDeclaredTypeOfSymbol(symbol: ts.Symbol, checker: ts.TypeChecker) {
	return checker.getDeclaredTypeOfSymbol(symbol)
}

export function getTypeOfSymbol(symbol: ts.Symbol, checker: ts.TypeChecker) {
...
interface TypeCheckerInternal extends ts.TypeChecker {
	...
	getAllPossiblePropertiesOfTypes(types: readonly Type[]): Symbol[]
	getUnionType(types: readonly Type[] /* there's another arg here that we don't need */): Type
}
...
export function getAllPossiblePropertiesOfTypes(types: ts.Type[], checker: ts.TypeChecker): ts.Symbol[] {
	const internalChecker = checker as TypeCheckerInternal
	return internalChecker.getAllPossiblePropertiesOfTypes(types)
}

export function createUnionType(types: ts.Type[], checker: ts.TypeChecker): ts.Type {
	const internalChecker = checker as TypeCheckerInternal
	return internalChecker.getUnionType(types)
}

export function isTypeAssignableTo(source: ts.Type, target: ts.Type, checker: ts.TypeChecker): boolean {
	const internalChecker = checker as TypeCheckerInternal
	return internalChecker.isTypeAssignableTo(source, target)
}
...
function inspectFlags(flags: number, flagType: Record<string, string | number>) {
	const parts: string[] = []
	for (const [key, value] of Object.entries(flagType)) {
...
export function inspectType(type: ts.Type, checker: ts.TypeChecker): string {
	const typeFlags = type.getFlags()
	let lines = `symbol.name: ${type.symbol?.name}`
	if (type.aliasSymbol) {
		lines += `, aliasSymbol.name: ${type.aliasSymbol.name}`
	}

	lines += `\ntypeToString(): ${indent(
		checker.typeToString(type, undefined, tsModule.TypeFormatFlags.NoTruncation | tsModule.TypeFormatFlags.MultilineObjectLiterals | tsModule.TypeFormatFlags.UseFullyQualifiedType)
	)}`

	const symbol = type.getSymbol()
	if (symbol) {
		lines += `\ngetSymbol(): ${indent(inspectSymbol(symbol, checker))}`
	}

	const aliasSymbol = type.aliasSymbol
	if (aliasSymbol) {
		lines += `\naliasSymbol: ${indent(inspectSymbol(aliasSymbol, checker))}`
	}

	lines += `\ntypeFlags: ${indent(inspectFlags(typeFlags, tsModule.TypeFlags))}`

	if ("objectFlags" in type) {
		const objectFlags = (type as any).objectFlags as number
		lines += `\nobjectFlags: ${indent(inspectFlags(objectFlags, tsModule.ObjectFlags))}`
	}

	const properties = type.getProperties()
	if (properties.length) {
		lines += `\ngetProperties(): ${properties.map(symbol => symbol.getName()).join(", ")}`
	}

	return renderInspect(`Type ${(type as any).id} (`, lines, `)`)
}

export function inspectSymbol(symbol: ts.Symbol, checker: ts.TypeChecker): string {
	const name = symbol.getName()
	const flags = inspectFlags(symbol.flags, tsModule.SymbolFlags)
	const asString = checker.symbolToString(symbol)
	return renderInspect(`Symbol ${(symbol as any).id} ${name} (`, `flags: ${flags}\nsymbolToString(): ${indent(asString)}`, ")")
}

const TAB = "  "
function indent(string: string, by = 1, firstLine = by) {
	const lines = string.split("\n")
	const first = lines.shift()
	const indentedFirst = first !== undefined ? TAB.repeat(firstLine) + first : undefined
	const indentedRest = lines.map(line => TAB.repeat(by) + line)
	return [indentedFirst, ...indentedRest].filter(isDefined).join("\n")
}

function renderInspect(openingLine: string, contents: string, closingLine: string) {
	return [openingLine, indent(contents), closingLine].join("\n")
}
