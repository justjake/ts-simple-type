...
const A_TYPES = process.env.TYPEA === undefined ? ALL_TYPES : process.env.TYPEA.split(";")
const B_TYPES = process.env.TYPEB === undefined ? ALL_TYPES : process.env.TYPEB.split(";")

const TOTAL_SPLITS = 8
const B_TYPE_CHUNKS = _.chunk(B_TYPES, Math.ceil(B_TYPES.length / TOTAL_SPLITS))

export function runTypeCombinationsShard(args: { index: number }) {
	const B_TYPES = B_TYPE_CHUNKS[args.index - 1]
